...
- **Task API** (`langchain_parallel.tasks`):
  - `ParallelTaskRunTool` — agent-callable `BaseTool` that runs a single Parallel Task synchronously via `client.task_run.execute(...)`. Surfaces the structured output, `basis` citations, and run id. Supports the full processor menu including the `-fast` family (`lite`, `lite-fast`, `base`, `base-fast`, `core`, `core2x`, `pro`, `ultra`, `ultra2x/4x/8x`, and matching `-fast` variants — 18 in total).
  - `ParallelDeepResearch` — `Runnable[str|dict, dict]` for the single-input, deep-research pattern. **Defaults to the `pro` processor** (Exploratory web research, 2-10 min); pass `processor="ultra"` for the most thorough multi-source reports (5-25 min). Per https://docs.parallel.ai/task-api/guides/choose-a-processor.
  - `ParallelTaskGroup` — low-level batch primitive: creates a Task Group, fans out runs, collects results. New `task_spec` field forwards as `default_task_spec` to `add_runs`. Default processor: `lite`.
  - `ParallelEnrichment` — `Runnable[list, list]` for the structured-batch enrichment pattern (typed counterpart to `ParallelDeepResearch`). Wraps `ParallelTaskGroup` with a `default_task_spec` built from pydantic input/output schemas; coerces pydantic instances → dicts on input. Default processor: `core` (the docs' recommendation for enrichment workflows).
